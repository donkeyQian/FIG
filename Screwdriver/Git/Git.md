...
`git branch` 命令可以查看本地仓库的所有分支，git 用 * 标识当前分支

![](../../image/Pasted%20image%2020230513141040.png)

>创建分支

`git branch [branchName]` 命令可以创建分支。

>切换分支

git 提供了两个切换分支的方式：
1. `git checkout [branchName]`
	1. checkout 命令还可以用来切换文件版本，如果文件名和分支名冲突，会引发混淆
2. `git switch [branchName]`（推荐）

>合并分支

`git merge [branchName]` 命令可以合并分支，合并的具体操作为以当前分支为主体，将目标分支的内容合并到当前分支

>解决冲突

合并分支时，如果两个分支的文件内容没有冲突，那么git 会为我们自动完成合并，但是如果两个分支修改了同一个文件的同一行内容，那么 git 就不知道应该保存那个分支的内容了，也就产生了冲突，这种情况就需要我们手动解决冲突。
